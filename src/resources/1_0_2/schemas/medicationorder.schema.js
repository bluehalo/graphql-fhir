	...
	fields: () => ({
		resourceType: {
			type: new GraphQLNonNull(
				new GraphQLEnumType({
					name: 'MedicationOrder_Enum_schema',
		...
		patient: {
			type: new GraphQLUnionType({
				name: 'MedicationOrderpatient_patient_Union',
				description:
		...
		prescriber: {
			type: new GraphQLUnionType({
				name: 'MedicationOrderprescriber_prescriber_Union',
				description:
		...
		encounter: {
			type: new GraphQLUnionType({
				name: 'MedicationOrderencounter_encounter_Union',
				description:
		...
		reasonReference: {
			type: new GraphQLUnionType({
				name: 'MedicationOrderreasonReference_reasonReference_Union',
				description:
					'Can be the reason or the indication for writing the prescription.',
		...
		medicationReference: {
			type: new GraphQLNonNull(
				new GraphQLUnionType({
					name: 'MedicationOrdermedicationReference_medicationReference_Union',
					description:
						'Identifies the medication being administered. This is a link to a resource that represents the medication which may be the details of the medication or simply an attribute carrying a code that identifies the medication from a known list of medications.',
		...
		dosageInstruction: {
			type: new GraphQLList(
				require('./medicationorderdosageinstruction.schema.js'),
			),
			description: 'Indicates how the medication is to be used by the patient.',
		},
		dispenseRequest: {
			type: require('./medicationorderdispenserequest.schema.js'),
			description:
				'Indicates the specific details for the dispense or medication supply part of a medication order (also known as a Medication Prescription).  Note that this information is NOT always sent with the order.  There may be in some settings (e.g. hospitals) institutional or system support for completing the dispense details in the pharmacy department.',
		},
		substitution: {
			type: require('./medicationordersubstitution.schema.js'),
			description:
				"Indicates whether or not substitution can or should be part of the dispense. In some cases substitution must happen, in other cases substitution must not happen, and in others it does not matter. This block explains the prescriber's intent. If nothing is specified substitution may be done.",
		},
		priorPrescription: {
			type: new GraphQLUnionType({
				name: 'MedicationOrderpriorPrescription_priorPrescription_Union',
				description:
